...
- SING_UF_PHANTOM_KEYS - набор может содержать удаленные (фантомные) ключи. Это полезно для схлопывания списков последовательных изменений набора в один без наличия 
исходного состояния Для такого набора недоступна операция сравнения с другим набором.  

_Флаги подключения - cм. sing_link_set:_
- SING_CF_MULTICORE_PARSE
- SING_CF_PARSE_ERRORS 
- SING_CF_FULL_LOAD
- SING_CF_READER 
- SING_CF_STAY_LOCKED - флаг, специфичный для sing_create_set. После создания и перед заполнением набора мутекс захватывается вызовом [sing_lock_W](#sing_lock_W), и 
остается захваченным до ручного вызова [sing_unlock_commit](#sing_unlock_commit)  
...
- SING_LM_SIMPLE - для блокировки записи используется мутекс. Запись из нескольких потоков в одном процессе приведет к неопред. поведению. 
Может быть обнаружен факт аварийного завершения пишущего процесса и восстановлена с дисковой копии консистентность набора данных. 
...
Модифицирующие вызовы в любом потоке без захваченного мутекса ожидают захвата мутекса процессом. Этот режим позволяет использовать несколько ядер для записи, 
не синхронизируя их между собой через мутекс. [sing_unlock_commit](#sing_unlock_commit) ожидает завершения всех модифицирующих вызовов перед освобождением мутекса.
- SING_LM_FAST - SING_LM_PROTECTED без проверки захвата мутекса. Можно использовать в хорошо отлаженном коде для скорости вместе с [sing_lock_W](#sing_lock_W) 
и [sing_unlock_commit](#sing_unlock_commit) или не использовать эти вызовы. Если вызовы не используются, ручная синхронизация с диском возможна через 
вызовы [sing_flush](#sing_flush) и [sing_revert](#sing_revert), сбой приведет к дедлоку записи.
- SING_LM_NONE - нет блокировок записи, т.к. запись более чем из одного потока одного процесса не планируется (приведет к повреждению данных). 
Тем не менее, можно выполнять ручную синхронизацию записи из нескольких процессов через [sing_lock_W](#sing_lock_W) и [sing_unlock_commit](#sing_unlock_commit). 
Если это не делается, то ручная синхронизация с диском возможна через вызовы [sing_flush](#sing_flush) и [sing_revert](#sing_revert).
- SING_LM_READ_ONLY - набор после создания не будет модифицироваться. Такой набор не требует открытых файловых дескрипторов для работы. Операции чтения работают немного быстрее.
...
- SING_CF_MULTICORE_PARSE - допустимо использовать дополнительные потоки при парсинге файлов. Вызовы работы с файлами (в т.ч. [sing_create_set](#sing_create_set)) будут выполняться быстрее.
...
```
отключается от набора данных. Разделяемый набор при этом остается в памяти машины, приватный набор данных уничтожается.  
**Параметры:**  
**_kvset_** - подключенный набор данных. После вызова невалиден.  

### sing_unload_set
```c
void sing_unload_set(FSingSet kvset)
```
выгружает разделяемый набор данных из памяти машины (наборы, созданные без дисковой копии уничтожаются).  
**Параметры:**  
**_kvset_** - подключенный набор данных. После вызова невалиден.  

### sing_delete_set
```c
void sing_delete_set(FSingSet \*kvset)
```
выгружает разделяемый набор данных из памяти машины и удаляет дисковую копию, если она есть.  
**Параметры:**  
**_kvset_** - подключенный набор данных. После вызова невалиден.  

## Работа с блокировками, синхронизация с диском

### sing_lock_W, sing_lock_RW
```c
int sing_lock_W(FSingSet *kvset)
int sing_lock_RW(FSingSet *kvset)
```
sing_lock_W захватывает мутекс блокировки записи для набора данных. Если предыдущий владелец не освободил его и завершился, откатывается к дисковой копии 
(или досохраняет данные в дисковую копию, если тот завершился во время синхронизации с диском). Работает во всех режимах блокировок кроме SING_LM_READ_ONLY.  
sing_lock_RW работает аналогично sing_lock_W, но останавливает читающие процессы, вводя их в цикл ожидания снятия блокировки. Сбой во время выполнения приведет к дедлоку 
и чтения и записи, снимаемому следующей модифицирующей операцией при использовании мутекса. Использовать этот вызов нежелательно.  
**Параметры:**  
...
SING_ERROR_IMPOSSIBLE_OPERATION - набор или подключение только для чтения, либо попытка выполнить повторную блокировку  
...
```
Выполняет синхронизацию с дисковой копией, если она есть, и снимает блокировки, наложенные sing_lock_W и sing_lock_RW.  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
...
**Возвращаемые значения:**  
0 - успех  
SING_ERROR_IMPOSSIBLE_OPERATION - набор не был заблокирован  
...
```
Выполняет откат к дисковой копии, если она есть, и снимает блокировки, наложенные sing_lock_W и sing_lock_RW.  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
**Возвращаемые значения:**  
0 - успех  
SING_ERROR_IMPOSSIBLE_OPERATION - у набора нет дисковой копии либо он не был заблокирован ранее  
...
0 - успех  
SING_ERROR_CONNECTION_LOST - набор удален  
SING_ERROR_IMPOSSIBLE_OPERATION - у набора нет дисковой копии или режим не SING_LM_FAST и не SING_LM_NONE, или набор заблокирован sing_lock_W и sing_lock_RW.   
SING_ERROR_SYNC_FAILED - сбой при синхронизации с диском (возможно кончилось место на диске). Синхронизация может быть завершена любым другим процессом, 
пока набор находится в памяти. После выгрузки из памяти использование набора станет невозможным. 
...
```
sing_add_file добавляет данные из файла в набор. Если ключ начинается с символа "-" - он удаляется из набора (либо создается фантомный ключ в наборе с фантомными ключами), ключи с символами "+" и "=" в начале и без символа операции добавляются, с символом "!" не обрабатываются. Если ключ уже есть в наборе, данные меняются на данные из файла.
...
на повторное копирование (например в интерфейсах для интерпретаторов), но не пишущие потоки. Простые вызовы возвращают значение встроенного типа или наличие ключа 
...
SING_RESULT_KEY_NOT_FOUND - ключ не найден, в этом случае \*value = NULL и \*vsize = 0  
...
которые помещаются в буфер целиком (не всегда порядке следования ключей)  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
**_keys_** - указатель на массив имен ключей  
**_ksizes_** - указатель на массив с размерами ключей  
...
**_results_** - место для сохранения наличия ключей. Должен указывать на память размером sizeof(int) \* count  
**Возвращаемые значения:**  
n >= 0 - число найденных и полностью сохраненных ключей. в этом случае:  
...
**Возвращаемые значения:**  
0 если ключ найден  
SING_RESULT_KEY_NOT_FOUND если ключ не найден  
...
```
Проверяет последовательно наличие нескольких ключей и сохраняет первые 32/64 бита значения в \*(values + i). Если ключ найден в наборе, results[i] равен 0, иначе 
содержит код причины. Вызов не атомарен.  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
**_keys_** - указатель на массив имен ключей  
**_count_** - число ключей  
...
```
sing_add_key добавляет ключ, если его нет в наборе, sing_set_key добавляет или перезаписывает ключ.  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
**_key_** - имя ключа  
...
**Возвращаемые значения:**  
0 - ключ добавлен  
SING_RESULT_KEY_PRESENT если ключ уже есть. sing_add_key в этом случае оставляет старое значение, а sing_set_key заменяет значение на переданное  
SING_RESULT_IMPOSSIBLE_KEY - невозможный для используемого кодека ключ  
...
```
удаляет ключи. Ключи обрабатываются не в порядке следования.  
**Параметры:**  
**_kvset_** - подключенный набор данных.  
**_keys_** - указатель на массив имен ключей  
**_count_** - число ключей  
...
&nbsp;&nbsp;\*(results + i) содержит SING_RESULT_KEY_NOT_FOUND если ключ уже есть в наборе  
&nbsp;&nbsp;\*(results + i) содержит SING_RESULT_IMPOSSIBLE_KEY если ключ невозможен для кодека  
SING_ERROR_NO_SET_MEMORY - не удается выделить страницу для новых данных  
SING_ERROR_CONNECTION_LOST - набор удален. В этом случае \*(value + i) не меняется  
